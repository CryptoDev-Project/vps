...
function build_mn_from_source() {
        ...
        if [ ! -f ${MNODE_DAEMON} ] || [ "$update" -eq 1 ]; then
                ...
                if [ "$update" -eq 1 ]; then
                    ...
                    if [ -f ${MNODE_DAEMON} ]; then
                            echo "UPDATE FAILED! Daemon ${MNODE_DAEMON} couldn't be removed. Please open an issue at https://github.com/CryptoDev-Project/vps/issues. Thank you!"
                            exit 1
                    fi
                fi
        ...
        if [ ! -f ${MNODE_DAEMON} ]; then
                echo "COMPILATION FAILED! Please open an issue at https://github.com/CryptoDev-Project/vps/issues. Thank you!"
                exit 1
        fi
}
